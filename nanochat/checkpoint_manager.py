...
def _patch_missing_config_keys(model_config_kwargs):
    ...
    if "train_recur_mean" in model_config_kwargs and "num_recur" not in model_config_kwargs:
        model_config_kwargs["num_recur"] = int(model_config_kwargs.pop("train_recur_mean"))
        log0("Migrated config key train_recur_mean -> num_recur")

    # Remove deprecated config keys
    deprecated_keys = [
        "recur_warm_start", "sandwich_norm", "train_recur_mean", "train_recur_max",
    ]
    for key in deprecated_keys:
        if key in model_config_kwargs:
            model_config_kwargs.pop(key)
            log0(f"Removed deprecated config key: {key}")
