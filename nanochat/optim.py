...
def muon_step_fused(
    ...
    p_norm: Tensor,                 # (12, 1, 1) - cached ||W_0||_F per param (constant)
    momentum_t: Tensor,             # () - 0-D CPU tensor, momentum coefficient
    lr_t: Tensor,                   # () - 0-D CPU tensor, learning rate
    ns_steps: int,                  # 5 - number of Newton-Schulz/Polar Express iterations
) -> None:
    """
    MuonH step: momentum -> polar_express -> norm_scale -> hyperball_update
    MuonH (Muon-Hyperball) constrains weights to a Frobenius sphere ||W||_F = c_W.
    The update is scaled to ||g||_F = c_W and weights are projected back to the sphere
    after each step. Weight decay is omitted (first-order no-op on the Frobenius sphere).
    Ref: arXiv:2603.28743, Microsoft ArchScale lit_gpt/optim/muon.py
    """
    ...
    # Scale update to ||g||_F = ||W_0||_F
    g_norm = g.norm(dim=(-2, -1), keepdim=True).clamp_min(1e-10)
    g = g * (p_norm.to(g.dtype) / g_norm)

    # Update and project back to Frobenius sphere
    lr = lr_t.to(g.dtype)
    stacked_params.sub_(lr * g)
    w_norm = stacked_params.norm(dim=(-2, -1), keepdim=True).clamp_min(1e-10)
    stacked_params.mul_(p_norm.to(stacked_params.dtype) / w_norm)
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    Arguments:
        param_groups: List of dicts, each containing:
            ...
            - For Muon groups: 'lr', 'momentum', 'ns_steps'
    """
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        # Cache ||W_0||_F once at first step (MuonH Frobenius sphere target)
        ...
        muon_step_fused(
            stacked_grads, stacked_params, momentum_buffer, state["p_norm"],
            self._muon_momentum_t, self._muon_lr_t, group["ns_steps"],
        )
    ...
    def _compute_muon(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        ...
        if num_owned > 0:
            ...
            muon_step_fused(
                ...
                state["momentum_buffer"][:num_owned], state["p_norm"][:num_owned],
                self._muon_momentum_t, self._muon_lr_t, group["ns_steps"],
            )
